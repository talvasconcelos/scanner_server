...
const utils = {
  ...
  calcReturns: (data) => {
    return data.map((c, i) => Math.tanh(((c - data[i - 1]) / data[i - 1]) * 100 || 0))
  },

  prepAiData: (data, rsi) => {
    data = getData(data)
    const returns = calcReturns(data.c)
    const rFactor = data.h
      .map((h, i) => rotationalFactor(h, data.h[i - 1], data.l[i], data.l[i - 1]))
    const nCandle = data.c
      .map((c, i) => {
        return [returns[i], rFactor[i], rsi[i] / 100]
      })
      .map((c, i, arr) => {
        if (i >= C.ws - 1) {
          let j = i - C.ws + 1
