...
const utils = {
  ...
  prepAiData: (raw, window = 12) => {
    ...
    const ma10 = utils.EMA(data.c, 10)
    const distFromMA = data.c.map((p, i) => (p - ma10[i]) / ma10[i])
    const zClose = utils.zscore(data.c)
    const zHigh = utils.zscore(data.h)
    const zLow = utils.zscore(data.l)
    const volOsc = utils.zscore(data.v)

    const X = data.c.map((c, i) => {
      return [g[i][0], g[i][1], squeeze[i], distFromMA[i], zClose[i], zHigh[i], zLow[i], volOsc[i]]
    }).map((c, i, arr) => {
      let j = window - 1    
